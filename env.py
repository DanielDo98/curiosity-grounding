...
class GroundingEnv:
    ...
    def reset(self):
        ...
        #image_rescaled = rescale(image, 1.0 / 4.0, anti_aliasing=False, multichannel=True) #TODO double check if channel first or last
        #When we're starting out, we don't have 5 frames, so just use 0 as placeholder frame.
        for _ in range(4):
            lerp_frames.append(np.zeros(screen_buf.shape))
        lerp_frames.append(screen_buf)

        state = (lerp_frames, self.instruction)
    ...
    def step(self, action_id):
        ...
        Returns:
           ...
           extra_args: Dictionary of additional arguments/parameters.
        """
        #We track this so our model can keep track of temporal dependencies of actions.
        lerp_frames = []
        ...
        if self.params.visualize:
            ...
            for _ in range(4):
                ...
                #image_rescaled = rescale(image, 1.0 / 4.0, anti_aliasing=False, multichannel=True)
                lerp_frames.append(screen_buf)
                # Slowing down the game for better visualization.
                sleep(self.params.sleep)
        else:
            for _ in range(4): #TODO this could be cleaned up
                self.game.make_action(actions[action_id], 1)
                screen = self.game.get_state().screen_buffer
                screen_buf = process_screen(
                    screen, self.params.frame_height, self.params.frame_width)
